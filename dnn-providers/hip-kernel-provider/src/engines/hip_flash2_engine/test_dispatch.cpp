...
struct Case { int B,H,S,D; bool c; const char* want; int wantSplit; };
...
int main()
{
    ...
    for(const auto& t : cases)
    {
        ...
        printf("%-26s %-10s %-10s %6d %6d  %s\n",lbl,t.want,s.variant.tag,
               t.wantSplit,s.splitK, ok?"ok":"MISMATCH");
        // geometry sanity: qPerCta must be waves*QG*16 and blockDim waves*64
        if(s.variant.blockDim % 64 != 0) { printf("   bad blockDim\n"); fail++; }
    }
    // workspace sanity
    ...
    printf("\nworkspace(B1 H8 S2048 D128 split4) = %zu bytes (expect %zu) %s\n",
           ws, expect, ws==expect?"ok":"MISMATCH");
    if(ws!=expect) fail++;
    printf("\n%s: %d mismatch(es)\n", fail? "FAIL":"PASS", fail);
    return fail?1:0;
}
